...
export default function About() {
  ...
  const infoCards = [
    { icon: <IconMapPin size={22} color="var(--sage-dark)" />, label: "Localisation", value: "Casablanca, Maroc", sub: "École Centrale Casablanca", bg: "var(--mint)" },
    { icon: <IconGraduate size={22} color="#7a6ab8" />, label: "Recherche", value: "Stage / Emploi", sub: "Finance & Stratégie", bg: "var(--lavender)" },
    { icon: <IconGlobe size={22} color="#3a6a8a" />, label: "Intérêt", value: "Marchés Financiers", sub: "BRVM / UEMOA", bg: "var(--sky-light)" },
    ...
    <>
      ...
      <section className="about" id="about" ref={sectionRef}>
        ...
        <div className="about-grid">
          <div>
            <p className="about-text reveal">Ce qui m'anime, c'est de comprendre en profondeur les mécanismes qui font avancer une économie — puis de les mettre au service de l'inclusion financière, pour que cette croissance profite aussi aux populations aujourd'hui exclues du système bancaire. J'aborde chaque mission avec la même rigueur analytique, qu'elle soit menée pour un cabinet, une banque ou mon propre projet entrepreneurial.</p>
            <div className="about-highlight reveal"><p>"Mon ambition : contribuer à faire franchir un cap à l'économie de mon pays, en construisant les outils d'une véritable inclusion financière pour toute la population."</p></div>
            <div className="reveal">
            ...
            {infoCards.map((card, i) => (
              <div key={i} className="info-card reveal">
                ...
                <div className="info-card-body">
                  ...
                  <div className="info-card-value" style={{ fontSize: card.label === "Email" ? "0.82rem" : undefined }}>{card.value}</div>
                  {card.sub && <div className="info-card-sub">{card.sub}</div>}
                </div>
              </div>
            ))}
          </div>
