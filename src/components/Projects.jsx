...
function ProjectCard({ project, index }) {
  ...
  return (
    <CardTag
      ...
      style={{ animationDelay: `${index * 100}ms`, background: hovered ? "white" : project.color, textDecoration: "none", cursor: project.link ? "pointer" : "default" }}
    ...
    >
      ...
      {project.link && (
        <div className="project-link-label">
          {project.linkLabel || "Voir plus"} <IconArrowRight size={13} color="var(--sage-dark)" />
        </div>
      )}
  ...
  return (
    <>
      <style>{`
        ...
        .project-card { border-radius: var(--radius-lg); padding: 32px; border: 1px solid rgba(232, 224, 208, 0.6); cursor: pointer; position: relative; overflow: hidden; transition: all 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94); display: block; color: inherit; }
        ...
        .project-link-label { display: inline-flex; align-items: center; gap: 6px; margin-top: 14px; font-size: 0.78rem; font-weight: 600; color: var(--sage-dark); }
